"""From the original Taskmaster files, extracts a clean dataset with utterances."""
import argparse
import json
import os
import random
...
def extract_data(input_directory, debug=False):
    ...
    for name in os.listdir(input_directory):
        ...
        for dialogue in tqdm(
            ...
            dialogue_id = dialogue["instruction_id"] + "_" + dialogue["conversation_id"]
            dialogue_data = []

            turn_id = 0
            speaker = dialogue["utterances"][0]["speaker"]
            utterance = []
            ...
            for turn in dialogue["utterances"]:
                ...
                utterance.append(turn["text"])

            # Add last group of turns to dialogue
            dialogue_data.append(
                {
                    "turn_id": turn_id,
                    "speaker": speaker if speaker != "ASSISTANT" else "SYSTEM",
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="From the original Taskmaster files, extracts a clean dataset with utterances."
    )
    parser.add_argument(
        "-i",
        "--input",
